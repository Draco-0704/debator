...
.workspace{display:grid;grid-template-columns:minmax(0,1.1fr) minmax(290px,.74fr);gap:1.35rem}.panel{padding:1.2rem 1.25rem;border:1px solid var(--line);border-radius:24px;background:linear-gradient(180deg,rgba(255,255,255,.045),rgba(255,255,255,.025));backdrop-filter:blur(10px)}
...
.verdict{padding:1.8rem;border:1px solid var(--line);border-radius:28px;background:linear-gradient(180deg,rgba(255,255,255,.05),rgba(255,255,255,.025)),linear-gradient(135deg,rgba(255,107,61,.12),transparent 58%);position:relative;overflow:hidden}
...
@media (max-width:980px){.hero-grid,.workspace{grid-template-columns:1fr}.hero{min-height:auto}.hero-side{border-left:none;border-top:1px solid rgba(255,255,255,.08);padding-left:0;padding-top:1.2rem;min-height:auto}.strip,.stats,.hero-rail,.debate{grid-template-columns:1fr 1fr}}
@media (max-width:640px){.block-container{padding-left:.9rem!important;padding-right:.9rem!important}.hero-grid{padding:0 1rem}.strip,.stats,.hero-rail,.debate{grid-template-columns:1fr}.row{grid-template-columns:36px 1fr}.tag,.pill,.conf{grid-column:2;justify-self:start}}
</style>
""",
...
def _render_sidebar() -> None:
    with st.sidebar:
        ...
        for item in reversed(st.session_state.get("history", [])[-5:]):
            st.caption(f"{item['verdict']} - {item['preview']}")
...
def _render_footer() -> None:
    st.markdown(
        '<div class="footer-wrap"><div>Fake News Debater - adversarial multi-agent article review.</div><div>Powered by <a href="https://groq.com" target="_blank" rel="noopener noreferrer">Groq</a>, <a href="https://spacy.io" target="_blank" rel="noopener noreferrer">spaCy</a>, and <a href="https://huggingface.co" target="_blank" rel="noopener noreferrer">HuggingFace</a>.</div></div>',
        unsafe_allow_html=True,
    )
